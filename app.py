...
# CSS customizado para garantir visibilidade total dos cards sem cortes
st.markdown("""
    <style>
    .metric-container {
        display: flex;
...
# -------------------------------------------------------------
# 1. CARDS DE DESEMPENHO E CONVERSÃO (LAYOUT LIMPO)
# -------------------------------------------------------------
...
with col1:
    st.markdown(f"""
        <div class="metric-card-custom">
            <div class="metric-card-title">📅 Sessões Agendadas</div>
            <div class="metric-card-value">{total_periodo}</div>
            <div class="metric-card-sub">Agenda do período</div>
        </div>
    """, unsafe_allow_html=True)
...
with col2:
    st.markdown(f"""
        <div class="metric-card-custom">
            <div class="metric-card-title">📊 Auditadas pela IA</div>
            <div class="metric-card-value">{total_analisadas}</div>
            <div class="metric-card-sub">Análises geradas</div>
        </div>
    """, unsafe_allow_html=True)
...
with col3:
    st.markdown(f"""
        <div class="metric-card-custom">
            <div class="metric-card-title">🟢 Conversão Total</div>
            <div class="metric-card-value">{total_convertidos}</div>
            <div class="metric-card-sub">⚡ {vendas_ato} Ato | ⏳ {vendas_fup} FUP</div>
        </div>
    """, unsafe_allow_html=True)
...
with col4:
    st.markdown(f"""
        <div class="metric-card-custom">
            <div class="metric-card-title">📈 Taxa de Conversão</div>
            <div class="metric-card-value">{taxa_conversao:.1f}%</div>
            <div class="metric-card-sub">🔴 {total_perdidos} Perdidos</div>
        </div>
    """, unsafe_allow_html=True)
...
with col5:
    st.markdown(f"""
        <div class="metric-card-custom">
            <div class="metric-card-title">⭐ Nota Média FHT</div>
            <div class="metric-card-value">{media_nota:.1f} / 10</div>
            <div class="metric-card-sub">Média das auditorias</div>
        </div>
    """, unsafe_allow_html=True)
...
# -------------------------------------------------------------
# 2. SELEÇÃO DE REUNIÃO E LEITURA DA COLUNA 'STATUS'
# -------------------------------------------------------------
...
if st.session_state["eventos_carregados"]:
    ...
    # Lógica de Leitura Estrita das Colunas da Planilha Base_Master
    status_master = "Perdido"
    closer_master = "Não identificado"
    objecao_master = "Sem objeção registrada"
    
    if not df_master.empty and "Cliente" in df_master.columns:
        lead_norm = remover_acentos(nome_lead_limpo)
        
        # Filtra procurando pelo nome do lead na coluna Cliente
        match = df_master[df_master["Cliente"].apply(remover_acentos).str.contains(lead_norm.split()[0], na=False)] if lead_norm else pd.DataFrame()
        
        if len(match) > 1 and len(lead_norm.split()) > 1:
            match_refinado = match[match["Cliente"].apply(remover_acentos).str.contains(lead_norm.split()[1], na=False)]
            if not match_refinado.empty:
                match = match_refinado

        if not match.empty:
            # Puxa EXATAMENTE as colunas do layout original da planilha: Status (Coluna O), Closer (Coluna N) e Objeção (Coluna Q)
            if "Status" in match.columns:
                val_status = str(match.iloc[0].get("Status", "Perdido")).strip()
                if val_status and val_status != "nan":
                    status_master = val_status

            if "Closer" in match.columns:
                val_closer = str(match.iloc[0].get("Closer", "Não identificado")).strip()
                if val_closer and val_closer != "nan":
                    closer_master = val_closer

            if "Objeção" in match.columns:
                val_obj = str(match.iloc[0].get("Objeção", "Sem objeção registrada")).strip()
                if val_obj and val_obj != "nan":
                    objecao_master = val_obj

    # Padronização Rígida de Status
    if "ato" in status_master.lower():
        status_master_padrao = "Ganho (Ato)"
        is_venda_confirmada = True
    elif "fup" in status_master.lower():
        status_master_padrao = "Ganho (FUP)"
        is_venda_confirmada = True
    else:
        status_master_padrao = "Perdido"
        is_venda_confirmada = False

    # Painel Informativo Fixo com Dados Reais da Planilha
    if is_venda_confirmada:
        st.markdown(f"""
            <div class="status-badge-green">
                🟢 <b>Status Real na Planilha Master:</b> {status_master_padrao} | <b>Closer:</b> {closer_master}
            </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
            <div class="status-badge-red">
                🔴 <b>Status Real na Planilha Master:</b> Perdido | <b>Closer:</b> {closer_master} | <b>Objeção:</b> {objecao_master}
            </div>
        """, unsafe_allow_html=True)
...
- Status Real Confirmado: {status_master_padrao} (VENDA CONVERTIDA)
- Closer Responsável: {closer_master}

REGRAS ABSOLUTAS:
1. O primeiro item da sua resposta DEVE SER O STATUS DESTACADO:
   `🟢 STATUS: LEAD CONVERTIDO ({status_master_padrao.upper()})`

2. A nota final OBRIGATORIAMENTE DEVE SER ENTRE 8.0 E 10.0. Elogie a execução e o fechamento.
...
"""
                    ...
                    st.session_state["historico_analises"].append({
                        ...
                        "status_venda": status_master_padrao,
                        "Objeção": objecao_master if not is_venda_confirmada else "Nenhuma / Fechado",
